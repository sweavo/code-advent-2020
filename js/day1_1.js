...
const find_pair = ( target, input ) => {
    for (let ii=0; ii<input.length; ++ii )
    {
        let sub_target=target-input[ii]
        if ( input.includes( sub_target ) ) 
        {
            return [ input[ii], sub_target ]
        }
    }
}
